...
function renderMessage(data, type)
{
    ...
    if (typeof data === 'string') {
        ...
        if (type == 'send') {
            ...
            var msgType = data.getElementsByTagName("MsgType")[0].firstChild.nodeValue;
            switch(msgType) {
                case 'text':
                    params.content = nl2br(data.getElementsByTagName("Content")[0].firstChild.nodeValue);
                    break;
                case 'news':
                    ...
                    if (title.length > 1) {
                        ...
                        for(var i = 0; i < title.length; i++) {
                            params.content.push({
                                title: data.getElementsByTagName("Title")[i].firstChild.nodeValue,
                                description: data.getElementsByTagName("Description")[i].firstChild.nodeValue,
                                picurl: data.getElementsByTagName("PicUrl")[i].firstChild.nodeValue,
                                url: data.getElementsByTagName("Url")[i].firstChild.nodeValue
                            });
                        }
                    }
                    break;
...
$('#wechatForm').submit(function(e){
    ...
    $.ajax($('[name=wechat]', _this).data('api-link'), {
        headers : {
        ...
        var receiveData = xml2dom(response);
        if(receiveData){
            var data = receiveData.getElementsByTagName("xml");
            if(data.length){
                renderMessage(data.item(0), 'receive');
            } else {
